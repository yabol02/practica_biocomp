...
def crossover_factory(name: str, **kwargs) -> Crossover:
    ...
    if name not in operators:
        raise ValueError(f"Crossover '{name}' no reconocido. Opciones: {list(operators.keys())}")
...
def initialization_factory(name: str, **kwargs) -> Initialization:
    ...
    name = name.lower().replace(" ", "_")
    if name not in operators:
        raise ValueError(f"Initialization '{name}' no reconocido. Opciones: {list(operators.keys())}")
...
def mutation_factory(name: str, **kwargs) -> Mutation:
    ...
    name = name.lower().replace(" ", "_")
    if name not in operators:
        raise ValueError(f"Mutation '{name}' no reconocido. Opciones: {list(operators.keys())}")
...
def replacement_factory(name: str, **kwargs) -> Replacement:
    ...
    name = name.lower().replace(" ", "_")
    if name not in operators:
        raise ValueError(f"Replacement '{name}' no reconocido. Opciones: {list(operators.keys())}")
