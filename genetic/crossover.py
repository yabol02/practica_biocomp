...
class PMXCrossover(Crossover):
    ...
    def cross(self, population: Population, n_offspring: int) -> Population:
        """
        Apply PMX crossover to population pairs.
        ...
        while len(new_individuals) < n_offspring:
            indices = random.sample(range(pop_size), 2)
            parent1 = population.individuals[indices[0]]
            parent2 = population.individuals[indices[1]]

            child1, child2 = self._pmx_individuals(parent1, parent2, genome_length)
            new_individuals.append(child1)
    ...
    def _pmx_individuals(
        self, parent1: Individual, parent2: Individual, genome_length: int
    ) -> Tuple[Individual, Individual]:
        """
        Perform PMX crossover between two parents.
        ...
        child_class = parent1.__class__
        return child_class(genotype=child1_gen, bounds=parent1.bounds), child_class(
            genotype=child2_gen, bounds=parent1.bounds
        )
...
class CycleCrossover(Crossover):
    ...
    def cross(self, population: Population, n_offspring: int) -> Population:
        """
        Apply Cycle crossover to population pairs.
        ...
        while len(new_individuals) < n_offspring:
            ...
            child1, child2 = self._cycle_individuals(parent1, parent2, genome_length)
            new_individuals.append(child1)
    ...
    def _cycle_individuals(
        ...
        c1 = np.full(genome_length, -1)
        c2 = np.full(genome_length, -1)

        visited = [False] * genome_length
        ...
        return child_class(genotype=c1, bounds=bounds), child_class(
            genotype=c2, bounds=bounds
        )
...
class EdgeRecombinationCrossover(Crossover):
    ...
    def cross(self, population: Population, n_offspring: int) -> Population:
        """
        Apply Edge Recombination crossover to population pairs.
        ...
        while len(new_individuals) < n_offspring:
            ...
            child1, child2 = self._erx_individuals(parent1, parent2)
            new_individuals.append(child1)
    ...
    def _erx_individuals(
        ...
        child1_gen = self._build_child(p1_gen, p2_gen)
        child2_gen = self._build_child(p2_gen, p1_gen)
    ...
    def _build_edge_map(self, p1, p2):
        """Build adjacency map for ERX."""
        edge_map = {}
        n = len(p1)
        for i in range(n):
            city = p1[i]
    ...
    def _build_child(self, p1, p2):
        """Build a child using ERX."""
        n = len(p1)
        edge_map = self._build_edge_map(p1, p2)
